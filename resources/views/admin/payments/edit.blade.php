                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.payments.update", [$payment->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('transaction_amount') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="transaction_amount" id="transaction_amount" value="{{ old('transaction_amount', $payment->transaction_amount) }}" step="1">
                        ...
                        <div class="form-group {{ $errors->has('earned_points') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="earned_points" id="earned_points" value="{{ old('earned_points', $payment->earned_points) }}" step="1">
                        ...
                        <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="status_id" id="status_id" required>
                                @foreach($statuses as $id => $status)
                                    <option value="{{ $id }}" {{ (old('status_id') ? old('status_id') : $payment->status->id ?? '') == $id ? 'selected' : '' }}>{{ $status }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('payment_method') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="payment_method_id" id="payment_method_id">
                                @foreach($payment_methods as $id => $payment_method)
                                    <option value="{{ $id }}" {{ (old('payment_method_id') ? old('payment_method_id') : $payment->payment_method->id ?? '') == $id ? 'selected' : '' }}>{{ $payment_method }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('note') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="note" id="note" value="{{ old('note', $payment->note) }}">
                            @if($errors->has('note'))
