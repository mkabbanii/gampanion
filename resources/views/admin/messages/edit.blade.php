                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.messages.update", [$message->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('message') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="message" id="message" value="{{ old('message', $message->message) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('sender') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="sender_id" id="sender_id" required>
                                @foreach($senders as $id => $sender)
                                    <option value="{{ $id }}" {{ (old('sender_id') ? old('sender_id') : $message->sender->id ?? '') == $id ? 'selected' : '' }}>{{ $sender }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('receiver') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="receiver_id" id="receiver_id" required>
                                @foreach($receivers as $id => $receiver)
                                    <option value="{{ $id }}" {{ (old('receiver_id') ? old('receiver_id') : $message->receiver->id ?? '') == $id ? 'selected' : '' }}>{{ $receiver }}</option>
                                @endforeach
                            </select>
