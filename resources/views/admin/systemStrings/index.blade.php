...
<div class="content">
    @can('system_string_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.system-strings.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $systemString->id }}">
                                        ...
                                        <td>
                                            @can('system_string_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('admin.system-strings.show', $systemString->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('system_string_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('admin.system-strings.edit', $systemString->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('system_string_delete')
                                                <form action="{{ route('admin.system-strings.destroy', $systemString->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('system_string_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.system-strings.massDestroy') }}",
